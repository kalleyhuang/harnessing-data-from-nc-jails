...
table(county_m$year, is.na(county_m$avg_felony_inmate_unsentenced))
table(county_m$census_county_name, is.na(county_m$avg_felony_inmate_unsentenced))

table(county_m$year, is.na(county_m$avg_misdemean_inmate_unsentenced))
...
#columns 24 to 30
num_inmate_get_mental_heath_bed
num_inmates_seen_sick_call
num_doctor_occurrences
num_offsite_medical_appointment
num_dental_encounters
avg_inmate_get_sick_bed
# columns 32 to 35
avg_own_inmate_housed_elsewhere
avg_fed_inmate_housed_contract
avg_state_inmate_housed_contract
avg_local_inmate_housed_contract
#columns 42 to 48
adp_of_maximum_security_inmates
adp_of_medium_security_inmates
adp_of_minimum_security_inmates
avg_inmates_get_medical_bed
avg_inmates_get_mental_heath_bed
avg_inmate_need_reg_med_attent
avg_inmates_need_reg_ment_health
```
The following counties are missing at least 10% of data for average number of unsentenced misdemeanor and felony inmates: Del Norte, Imperial, Inyo, Lassen, Monterey, Napa, San Benito, San Diego, Santa Barbara, Santa Clara, Santa Cruz, Shasta, Tehama, Trinity, Tulare, Tuolumne, and Ventura. Tehama and Santa Barbara are missing, respectively, at least 96% and 42% of data for those variables. Although missingness statewide is slightly concentrated before 2001, it is more concentrated by county.

started recording  2002 onwards
...
county_q %>% 
  ...
  count(census_county_name)
```
The following variables were not counted after 2001: number of inmates who are 2nd strikers, number of inmates who are 3rd strikers, number of unserved misdemeanor warrants, number of unserved felony warrants, and percent of inmates who are undocumented immigrants. I subset the data, filtering for observations before 2001 and grouping by county. 

The following variables were not counted before 2001: average length of stay for pre-trial release, average length of stay for sentenced release, average length of stay for all releases, money spent on psychiatric medicine last quarter, and money spent on medication last quarter. I subset the data, filtering for observations after 2001 and grouping by county. In general, I try to identify randomness or concentration of missingness greater than 0.1.
