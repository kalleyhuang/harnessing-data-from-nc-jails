...
```{r length_of_stay_viz, fig.width = 6}
length_of_stay %>% 
  group_by(date) %>% 
...
realignment_length_of_stay %>% 
  group_by(year(date)) %>% 
  summarize(pretrial_q25 = quantile(pretrial_change, 0.25, na.rm = TRUE), 
            pretrial_median = median(pretrial_change, na.rm = TRUE),
            pretrial_q75 = quantile(pretrial_change, 0.75, na.rm = TRUE),
            sentenced_q25 = quantile(sentenced_change, 0.25, na.rm = TRUE), 
            sentenced_median = median(sentenced_change, na.rm = TRUE),
            sentenced_q75 = quantile(sentenced_change, 0.75, na.rm = TRUE)) %>% 
  rename(year = `year(date)`)
```

We visualize the difference between lengths of stay after realignment.

```{r length_of_stay_realignment_viz}
