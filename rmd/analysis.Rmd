...
county_m %>% 
  ...
  filter(month(date) == 1) %>% # to control for seasonal effects
  arrange(date) %>% 
  select(date, county, total) %>% 
  group_by(county) %>% 
  unique() %>% 
  mutate(one_yr_pct_change = (total / lag(total) - 1) * 100,
         three_yr_pct_change = (total / lag(total) - 3) * 100, # THIS DOES NOT WORK
         five_yr_pct_change = (total / lag(total) - 5) * 100) # THIS DOES NOT WORK
# find mean percent change, look at percentiles (75% to 100%)
