...
population %>% 
  ...
  group_by(date) %>% 
  summarize(q25 = quantile(ratio, 0.25, na.rm = TRUE), q75 = quantile(ratio, 0.75, na.rm = TRUE),
            median = median(ratio))
```
...
# ~10 counties have less than 10% missingness
county_q %>% 
  filter(year > 2001) %>% 
  group_by(county) %>%  
...
median(length_of_stay_no_filter$pretrial_release - length_of_stay_filter$pretrial_release) # 1 day diff
median(length_of_stay_no_filter$sentenced_release - length_of_stay_filter$sentenced_release) # 20 day diff
